...
test('share link renders the REAL authentic verdict + a safe entity link (C1/C2/M3)', async ({ page }) => {
  await mockVerdict(page, { signature: 'valid', signer: 'verified', verdict: 'authentic', entity: { name: 'BBC', url: 'https://bbc.com' } });
...
test('forged share-link verdict strips an unsafe entity link (M3)', async ({ page }) => {
  await mockVerdict(page, { signature: 'valid', signer: 'mismatch', verdict: 'forged', entity: { name: 'EvilCorp', url: 'javascript:alert(1)' } });
...
test('no hardcoded authenticity — a tampered share-link result renders as modified', async ({ page }) => {
  await mockVerdict(page, { signature: 'modified', signer: 'unregistered', verdict: 'tampered' });
  ...
  await expect(page.locator('#verdict-banner')).toHaveClass(/tampered/);
});
